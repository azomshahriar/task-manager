...
public class UserController {
    ...
    @Operation(summary = "List of User", description = "", tags={ "user" })
    @GetMapping
    public ResponseEntity<Page<UserDto>> getUsers(Pageable pageable){ ;
        return ResponseEntity.ok().body(userService.getUsers(pageable));
    }
    @Operation(summary = "Get a user by userName", description = "", tags={ "user" })
    @GetMapping("/{userName}")
    public ResponseEntity<UserDto> getUser(@PathVariable(name = "userName",required = true) String userName){
