...
public class AdminController {
    ...
    @Operation(summary = "List of Task By User", description = "", tags={ "admin" })
    @GetMapping("/task/byUser/{userId}")
    public ResponseEntity<Page<TaskDto>> getTaskByProject(@PathVariable(name = "userId",required = true) Long userId, Pageable pageable){
        log.info("Get Task By project. Project Id:{}.");
        return ResponseEntity.ok(taskService.findTaskByUser(userId,pageable));
    }

    @Operation(summary = "List of Project", description = "", tags={ "admin" })
    @GetMapping("/project/byUser/{userId}")
    public ResponseEntity<Page<ProjectDto>> getProjectByUser(@PathVariable(name = "userId",required = true) Long userId,Pageable pageable){
