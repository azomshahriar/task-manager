package com.cardinality.taskmanager.integration;
...
class TaskManagerApplicationTests {
	...
	public void adminLoginTest() throws Exception{
		...
		JSONObject loginRequest = new JSONObject().put("username","admin").put("password","87654321");
		String url = this.baseUrl+this.loginUrl;
		System.out.println("Calling:"+url);
		HttpHeaders headers = new HttpHeaders();
		headers.setContentType(MediaType.APPLICATION_JSON);
		HttpEntity<String> request =
				new HttpEntity<String>(loginRequest.toString(), headers);
		ResponseEntity<JwtResponse> responseEntity =restTemplate.postForEntity(url,request,
				JwtResponse.class);
	...
	public void createUserTest() throws Exception{
		...
		ResponseEntity responseEntity =restTemplate.postForEntity(this.baseUrl+"/api/v1/admin/user",request,
				String.class);
		Assertions.assertEquals(HttpStatus.OK,responseEntity.getStatusCode());
		userDto.setUserName(user2);
		request =
				new HttpEntity<String>(objectMapper.writeValueAsString(userDto), headers);
		 responseEntity =restTemplate.postForEntity(this.baseUrl+"/api/v1/admin/user",request,
				String.class);
		Assertions.assertEquals(HttpStatus.OK,responseEntity.getStatusCode());
	...
	public void checkOnlyAdminCanCreateUser() throws Exception{
		JSONObject loginRequest = new JSONObject().put("username",user1).put("password","87654321");
		...
		HttpEntity<String> request =
				new HttpEntity<String>(loginRequest.toString(), headers);
		ResponseEntity<JwtResponse> responseEntity =restTemplate.postForEntity(loginUrl,request,
				JwtResponse.class);
		Assertions.assertEquals(HttpStatus.OK,responseEntity.getStatusCode());
		...
		final HttpEntity<String> request1 = new HttpEntity<String>(objectMapper.writeValueAsString(userDto), headers);

		     Assertions.assertThrows(Exception.class,()->{
				 ResponseEntity response =restTemplate.postForEntity(this.baseUrl+"/api/v1/admin/user",request1,
						 String.class);
			 });
	...
	public void createProjectAndTask_Corresponding_User() throws Exception{
		...
		Page<ProjectDto> adminProject= getUserProjects(this.baseUrl+"/api/v1/project",authToken);
		Page<ProjectDto> user1Project= getUserProjects(this.baseUrl+"/api/v1/project",getTokenUser(user1));
		Page<ProjectDto> user2Project= getUserProjects(this.baseUrl+"/api/v1/project",getTokenUser(user2));
		...
		Page<TaskDto> user1Tasks= getUserTask(this.baseUrl+"/api/v1/project",getTokenUser(user1));
		Page<TaskDto> user2Tasks= getUserTask(this.baseUrl+"/api/v1/project",getTokenUser(user2));
	...
	public Page<ProjectDto> getUserProjects(String url, String token){
		...
		ResponseEntity<RestResponsePage<ProjectDto>> responseEntity =restTemplate.exchange(
				url, HttpMethod.GET, request, new ParameterizedTypeReference<RestResponsePage<ProjectDto>>() {});
	...
	public Page<TaskDto> getUserTask(String url, String token){
		...
		ResponseEntity<RestResponsePage<TaskDto>> responseEntity =restTemplate.exchange(
				url, HttpMethod.GET, request, new ParameterizedTypeReference<RestResponsePage<TaskDto>>() {});
		return responseEntity.getBody();
	}
	...
	public void createObject(Object object, String url, String token) throws Exception{
		...
		ResponseEntity responseEntity =restTemplate.postForEntity(url,request,
				JwtResponse.class);
	}

	public String getTokenUser(String userName) throws Exception {
		JSONObject loginRequest = new JSONObject().put("username",userName).put("password","87654321");
		String loginUrl = this.baseUrl+this.loginUrl;
	...
	@Test
	@Order(5)
	@Sql(executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD, scripts = "classpath:update-due-date.sql")
